...
echo '{
  "tool_input": {
    "url": "https://api.flutter.dev/flutter/widgets/PopScope-class.html",
    "prompt": "extract the description"
  },
  "tool_response": "PopScope: a widget that controls whether the enclosing route can be popped"
...
3. Ask the agent to fetch a documentation page (e.g. "fetch `https://api.flutter.dev/flutter/widgets/PopScope-class.html` and summarize").
...
- **Servers without `ETag` or `Last-Modified` are never cached.** Most official doc sites (api.flutter.dev, docs.flutter.dev, dart.dev, docs.djangoproject.com, developer.mozilla.org) emit validators. Sites that don't are always re-fetched: pub.dev package pages, for example, returned neither header when last checked, so package documentation there is never cached.
